...
public:
                           ...
                           Scalar& step, Scalar& fx, Vector& grad, Scalar& dg, Vector& x)
    {
        ...
        if (param.linesearch != LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE)
            Log::REFatal("GPModel lbfgs: 'param.linesearch' must be 'LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE' for LineSearchNocedalWright");
        ...
        for (;;)
        {
            ...
            if (iter >= param.max_linesearch)
            {
                // throw std::runtime_error("the line search routine reached the maximum number of iterations");
                if (step_lo <= Scalar(0))
                    Log::REFatal("GPModel lbfgs: the line search routine failed, unable to sufficiently decrease the function value");
