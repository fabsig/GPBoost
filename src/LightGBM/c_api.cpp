		...
		void CheckParamConflictREModel(REModel* re_model = nullptr) {
			if (re_model != nullptr) {
				if (config_.boosting != std::string("gbdt")) {
					Log::Fatal("Gaussian process boosting can currently only be done for 'boosting = \"gbdt\"'");
				}
				if (config_.bagging_freq != 0) {
					Log::Fatal("Bagging cannot be applied when doing Gaussian process boosting. Set 'bagging_freq = 0'");
				}
				//make sure that objective for boosting and likelihood for re_model match
				...
				//if (!re_model->GaussLikelihood()) {//DELETE
				//	std::vector<std::string>::iterator it = std::find(config_.metric.begin(), config_.metric.end(), "l2");
				//	if (it != config_.metric.end()) {
				//		*it = "approx_neg_marginal_log_likelihood";
				//	}
				//	//config_.metric.insert(config_.metric.begin(), "approx_neg_marginal_log_likelihood");
				//}
...
int LGBM_BoosterUpdateOneIterCustom(BoosterHandle,
	const float*,
	const float*,
	int*) {
	API_BEGIN();
	...
	//#else//we always use double for score in GPBoost, so this is disabled
	//Booster* ref_booster = reinterpret_cast<Booster*>(handle);
