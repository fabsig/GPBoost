...
class RegressionL2loss: public ObjectiveFunction {
 public:
  ...
  void GetGradients(const double* score, score_t* gradients,
    ...
    if (weights_ == nullptr) {
      ...
      if (has_gp_model_) {
        if (train_gp_model_cov_pars_) {
          re_model_->OptimCovPar(gradients, false);
          re_model_->CalcGetYAux(gradients, true);
        }
        else {
          re_model_->CalcGetYAux(gradients, calc_cov_factor_);
          calc_cov_factor_ = false;
        }
      }
    } else {
      #pragma omp parallel for schedule(static)
 ...
 protected:
  ...
  /*! \brief Indicates whether the covariance matrix should also be factorized when calling re_model_->CalcGetYAux(). Only relevant if has_gp_model_ = true and train_gp_model_cov_pars_ = true */
