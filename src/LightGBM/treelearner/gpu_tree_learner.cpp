...
namespace LightGBM {
	...
	void PrintHistograms(HistogramBinEntry* h, size_t size) {
		...
		printf("\nTotal examples: %lu\n", total);
#else
			Rprintf("%03lu=%9.3g,%9.3g,%7d\t", i, h[i].sum_gradients, h[i].sum_hessians, h[i].cnt);
			total += h[i].cnt;
			if ((i & 3) == 3)
				Rprintf("\n");
	}
...
#else
		Rprintf("Computing histogram for %d examples and (%d * %d) feature groups\n", leaf_num_data, dword_features_, num_dense_feature4_);
		Rprintf("We can have at most %d workgroups per feature4 for efficiency reasons.\n"
			"Best workgroup size per feature for full utilization is %d\n", static_cast<int>(ceil(t)), (1 << exp_workgroups_per_feature));
#endif
#endif
...
#else
		Rprintf("Setting exp_workgroups_per_feature to %d, using %u work groups\n", exp_workgroups_per_feature, num_workgroups);
		Rprintf("Constructing histogram with %d examples\n", leaf_num_data);
#endif
#endif
...
#else
				Rprintf("feature-group %d using multiplier %d\n", i, device_bin_mults_.back());
#endif
#endif
				k++;
			}
...
#pragma omp parallel for schedule(static)
		for (int i = 0; i < static_cast<int>(dense_feature_group_map_.size() / dword_features_); ++i) {
			...
			printf("to devices\n");
#else
			Rprintf("Copying feature group ");
			for (int l = 0; l < dword_features_; ++l) {
				Rprintf("%d ", dense_ind[l]);
			}
			Rprintf("to devices\n");
#endif
#endif
			...
			printf("\n");
#else
			Rprintf("first example of feature-group tuple is: %d %d %d %d\n", host4[0].s[0], host4[0].s[1], host4[0].s[2], host4[0].s[3]);
			Rprintf("Feature-groups copied to device with multipliers ");
			for (int l = 0; l < dword_features_; ++l) {
				Rprintf("%d ", dev_bin_mult[l]);
			}
			Rprintf("\n");
#endif
#endif
...
#pragma omp parallel for schedule(static)
		...
		printf("\n");
#else
		Rprintf("Dense feature group list (size %lu): ", dense_feature_group_map_.size());
		for (int i = 0; i < num_dense_feature_groups_; ++i) {
			Rprintf("%d ", dense_feature_group_map_[i]);
		}
		Rprintf("\n");
		Rprintf("Sparse feature group list (size %lu): ", sparse_feature_group_map_.size());
		for (int i = 0; i < num_feature_groups_ - num_dense_feature_groups_; ++i) {
			Rprintf("%d ", sparse_feature_group_map_[i]);
		}
		Rprintf("\n");
#endif
#endif
...
#else
		Rprintf("Copying intial full gradients and hessians to device\n");
#endif
#endif
...
#else
			Rprintf("Using bagging, examples count = %d\n", cnt);
#endif
#endif
...
#else
			Rprintf("Indices size %d being copied (left = %d, right = %d)\n", end - begin, num_data_in_left_child, num_data_in_right_child);
#endif
#endif
...
#else
		Rprintf("Feature masks:\n");
		for (unsigned int i = 0; i < feature_masks_.size(); ++i) {
			Rprintf("%d ", feature_masks_[i]);
		}
		Rprintf("\n");
		Rprintf("%d feature groups, %d used, %d\n", num_dense_feature_groups_, used_dense_feature_groups, use_all_features);
#endif
#endif
...
#else
			Rprintf("Comparing histogram for feature %d size %d, %lu bins\n", dense_feature_group_index, num_data, size);
#endif
...
#else
			Rprintf("Feature %d smaller leaf:\n", feature_index);
			PrintHistograms(smaller_leaf_histogram_array_[feature_index].RawData() - 1, bin_size);
			if (larger_leaf_splits_ == nullptr || larger_leaf_splits_->LeafIndex() < 0) { continue; }
			Rprintf("Feature %d larger leaf:\n", feature_index);
#endif
...
#else
		Rprintf("Splitting leaf %d with feature %d thresh %d gain %f stat %f %f %f %f\n", best_Leaf, best_split_info.feature, best_split_info.threshold, best_split_info.gain, best_split_info.left_sum_gradient, best_split_info.right_sum_gradient, best_split_info.left_sum_hessian, best_split_info.right_sum_hessian);
#endif
#endif
