...
namespace GPBoost {
	...
	*/
	...
	double EvalLLforOptimLib(const vec_t& pars,
		...
		if (objfn_data->optimizer_ == "nelder_mead" && gradient != nullptr) {//"hack" for printing nice logging information with nelder_mead
			if (has_covariates) {
			...
			Log::REDebug("GPModel: parameters after optimization iteration number %d: ", (int)(*gradient)[0]);
			re_model_templ_->PrintTraceParameters(cov_pars, beta, aux_pars.data());
			if (re_model_templ_->GetLikelihood() == "gaussian") {
				Log::REDebug("Negative log-likelihood: %g", (*gradient)[1]);
			}
			else {
				Log::REDebug("Approximate negative marginal log-likelihood: %g", (*gradient)[1]);
			}
		}//end only PrintTraceParameters()
		else {//not only PrintTraceParameters()
			if (has_covariates) {
				beta = pars.segment(num_cov_pars_optim, num_covariates);
		...
		}//end not only PrintTraceParameters()
	...
	public:
		...
		double operator()(const vec_t& pars, vec_t& gradient) {
			...
			// Determine number of covariance and linear regression coefficient parameters
			int num_cov_pars_optim, num_covariates, num_aux_pars;
			if (learn_covariance_parameters_) {
				num_cov_pars_optim = re_model_templ_->GetNumCovPar();
			...
			// Calculate gradient
			vec_t grad_cov;
			if (learn_covariance_parameters_ || re_model_templ_->EstimateAuxPars()) {
				re_model_templ_->CalcGradCovParAuxPars(cov_pars, grad_cov, gradient_contains_error_var, false, fixed_effects_ptr);
			}
			if (learn_covariance_parameters_) {
				gradient.segment(0, num_cov_pars_optim) = grad_cov.segment(0, num_cov_pars_optim);
	...
	*/
	...
	void OptimExternal(REModelTemplate<T_mat, T_chol>* re_model_templ,
		...
		if (optimizer == "nelder_mead") {
			...
			EvalLLforLBFGSpp<T_mat, T_chol> ll_fun(re_model_templ, fixed_effects, learn_covariance_parameters,
