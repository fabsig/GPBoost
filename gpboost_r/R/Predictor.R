...
Predictor <- R6::R6Class(
  ...
  public = list(
    ...
    finalize = function() {
      ...
      if (private$need_free_handle && !gpb.is.null.handle(private$handle)) {
    ...
    initialize = function(modelfile, ...) {
      params <- list(...)
      private$params <- gpb.params2str(params)
      ...
      if (is.character(modelfile)) {
        ...
        # Create handle on it
        handle <- gpb.call("LGBM_BoosterCreateFromModelfile_R", ret = handle, gpb.c_str(modelfile))
        private$need_free_handle <- TRUE

      } else if (methods::is(modelfile, "gpb.Booster.handle")) {
        ...
        stop("Predictor: modelfile must be either a character filename or an gpb.Booster.handle")
    ...
    current_iter = function() {
      ...
      cur_iter <- 0L
      gpb.call("LGBM_BoosterGetCurrentIteration_R",  ret = cur_iter, private$handle)
    ...
    predict = function(data,
      ...
      if (identical(class(data), "character") && length(data) == 1) {
        ...
        # Data is a filename, create a temporary file with a "gpboost_" pattern in it
        tmp_filename <- tempfile(pattern = "gpboost_")
        on.exit(unlink(tmp_filename), add = TRUE)

        # Predict from temporary file
        gpb.call("LGBM_BoosterPredictForFile_R", ret = NULL, private$handle, data,
          as.integer(header),
          as.integer(rawscore),
          as.integer(predleaf),
        ...
        npred <- gpb.call("LGBM_BoosterCalcNumPredict_R",
                          ret = npred,
                          private$handle,
                          as.integer(num_row),
                          as.integer(rawscore),
        ...
        if (is.matrix(data)) {
          preds <- gpb.call("LGBM_BoosterPredictForMat_R",
                            ret = preds,
                            private$handle,
                            data,
          ...
          preds <- gpb.call("LGBM_BoosterPredictForCSC_R",
                            ret = preds,
                            private$handle,
                            data@p,
                            data@i,
          ...
          # to-do: predict from gpb.Dataset
