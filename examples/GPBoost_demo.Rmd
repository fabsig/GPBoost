...
sigma2_1 <- 0.5^2 # marginal variance of GP
rho <- 0.1 # range parameter
sigma2 <- 0.25^2 # error variance
...
coords_test <- cbind(s_1=x1,s_2=x2)
...
```{r plot_data, echo=F, results='hide', message=F, cache=cache, warning=FALSE, fig.align='center', fig.cap='Illustration of data: Mean function F(X) used for simulation, "true" latent Gaussian process b, and observed data y (bottom plots)'}
library(ggplot2)
library(viridis)
library(gridExtra)
...
bst <- gpboost(data = X, label = y,
               gp_model = gp_model,
               nrounds = 207, learning_rate = 0.01,
               max_depth = 3, min_data_in_leaf = 5, num_leaves = 2^10,
...
params = { 'objective': 'regression_l2', 'learning_rate': 0.01,
            'max_depth': 3, 'min_data_in_leaf': 5, 'num_leaves': 2**10, 'verbose': 0 }
bst = gpb.train(params=params, train_set=data_train,
                gp_model=gp_model, num_boost_round=207)
print("Estimated covariance parameters:")
gp_model.summary()
```
</div>
...
print(paste0("Mean square error: ", MSE_GPBoost))# 0.2678827
```
Plot predictions
<div class="fold s o">
...
plot_data <- data.frame(s_1=coords_test[,1],s_2=coords_test[,2],
                        pred_mean=pred$random_effect_mean,
                        pred_sd=sqrt(pred$random_effect_cov),
                        true_GP=b_all[1:nx^2])
...
plot5 <- ggplot(data = plot_data, aes(x=s_1,y=s_2,color=pred_mean)) +
  geom_point(size=2, shape=15) + scale_color_viridis(option = "B") + ggtitle("Predicted GP mean")
plot6 <- ggplot(data = plot_data, aes(x=s_1,y=s_2,color=pred_sd)) +
  geom_point(size=2, shape=15) + scale_color_viridis(option = "B") + 
  labs(title="Predicted GP standard deviation", subtitle=" = prediction uncertainty")
...
plot8 <- ggplot(data = plot_data, aes(x=s_1,y=s_2,color=true_GP)) + 
...
```{r show_plot_prediction, echo=FALSE, results='hold', message=F, cache=cache, fig.align='center', fig.cap='Prediction: Predicted (posterior) mean and prediction uncertainty (=standard deviation) of GP as well as predicted mean function F(X).'}
grid.arrange(plot8, plot5, plot6, plot7, ncol=2)
```
...
A trained model can be interpreted using various tools. Besides classical feature importance measures and partial dependence plots, SHAP values and dependence plots can be created as follows. *Note: for the R package, you need version 0.4.3 or latter for creating these SHAP plots.*

***In R***
```{r interpretation, echo=TRUE, warning=FALSE, results='hold', message=F, cache=cache}
library("SHAPforxgboost")
shap.plot.summary.wrap1(bst, X = X)
shap_long <- shap.prep(bst, X_train = X)
shap.plot.dependence(data_long = shap_long, x = "Var_1")
```
...
#   negative likelihood of the GP model can be very flat which can cause problems for Fisher scoring.
...
# Use a random grid search. To use a deterministic grid search and try all possible
#   combinations of parameters, set 'num_try_random = NULL'
set.seed(1)
opt_params <- gpb.grid.search.tune.parameters(param_grid = param_grid,
                                              ...
                                              eval = "l2")
# I obtained the following result on my computer:
# ***** New best score (0.238948790779722) found for the following parameter combination: learning_rate: 0.01, min_data_in_leaf: 5, max_depth: 3, nrounds: 207
```
...
# Use a random grid search. To use a deterministic grid search and try all possible
#   combinations of parameters, set 'num_try_random=None'
...
In the following, we compare the GPBoost algorithm to plain gradient tree-boosting with a squared loss, to a linear Gaussian process model, and to random forest. For gradient boosting and random forest, we add the coordinates to the predictor variables (features) for the trees. For the linear Gaussian process, we include the predictor variables in a linear regression term. As the results below show, **plain gradient tree-boosting, random forest, and a linear Gaussian process model result in considerably larger mean square errors (MSE) compared to the GPBoost algorithm**.
...
print(paste0("MSE of linear Gaussian process model: ", MSE_lin))# 1.177665
...
#                                               eval = "l2")
# I obtained the following result on my computer:
# ***** New best score (0.36223140526897) found for the following parameter combination: learning_rate: 0.01, min_data_in_leaf: 10, max_depth: 3, nrounds: 496
bst <- gpboost(data = XC, label = y,
               nrounds = 496, learning_rate = 0.01,
...
We first simulate data and then show how to train a GPBoost model and make predictions.
<div class="fold s">
