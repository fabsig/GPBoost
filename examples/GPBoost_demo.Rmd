...
# Modeling Background
It is assumed that the data y is the sum of a non-linear mean function $F(X)$ and so-called random effects $Zb$:
 $$ y = F(X) + Zb + \xi,$$
...
The **model is trained using the GPBoost algorithm**, where trainings means estimating the variance and **covariance parameters** of the random effects and the **mean function F(X) using a tree ensemble**. In brief, the GPBoost algorithm iteratively estimates the covariance parameters and adds a tree to the ensemble using gradient boosting steps. Trees are learned using the [LightGBM](https://github.com/microsoft/LightGBM/) library. See the [reference paper](XXX) for more details.
...
## Data
First, you need to load your data. For simplicity and illustrational purposes, we use simulated data here. We simulate data Using  R and illustrate the data in the figure below.
<div class="fold s">
```{r simulate_data, results='hide', message=F, cache=save_cache}
# Simulate data Using  R
...
```{r plot_data, echo=F, results='hide', message=F, cache=save_cache, fig.align='center', fig.cap='Illustration of data: Mean function F(X) used for simulation, latent Gaussian process b, and observed data y (bottom plots)'}
library(ggplot2)
library(viridis)
library(gridExtra)
...
***Using  R***
```{r training, results='hold', message=F, cache=save_cache, eval=T}
...
#   form of Newton's method and does not require  a learning rate
gp_params <- list(optimizer_cov = "gradient_descent", lr_cov = 0.05, trace = FALSE,
                  use_nesterov_acc = TRUE, acc_rate_cov = 0.5, delta_rel_conv = .Machine$double.eps^0.25)
...
**Using  Python**
<div class="fold s o">
```{python training_python, collapse=T, message=F, cache=save_cache, eval=run_python}
# 1. Create Gaussian process / random effects model
...
gp_model.set_optim_params(params={"optimizer_cov": "gradient_descent", "lr_cov": 0.05,
                                  ...
                                  "delta_rel_conv": np.finfo(float).eps**0.25, "trace": False})
...
***Using  R***
```{r prediction, results='hold', message=F, cache=save_cache, fig.align='center', fig.cap='Prediction: Predicted (posterior) mean and prediction uncertainty (=standard deviation) of GP as well as predicted mean function F(X).'}
# make predictions
...
plot7 <- ggplot(data=data.frame(x=X_test[,1],f=pred$fixed_effect), aes(x=x,y=f)) + geom_line(size=1) +
  geom_line(data=data.frame(x=x,f=f1d(x)), aes(x=x,y=f), size=1.5, color="darkred") +
  ggtitle("Predicted F(X)") + xlab("X") + ylab("y")
...
**Using  Python**
<div class="fold s o">
```{python prediction_python, collapse=T, message=F, cache=save_cache, eval=run_python}
...
***Using  R***
```{r cv, results='show', message=F, cache=save_cache, eval=run_cv}
gp_model <- GPModel(gp_coords = coords, cov_function = "exponential")
...
**Using  Python**
<div class="fold s o">
```{python cv_python, collapse=T, message=F, cache=save_cache, eval=run_python}
gp_model = gpb.GPModel(gp_coords=coords, cov_function="exponential")
gp_model.set_optim_params(params={"optimizer_cov": "gradient_descent", "lr_cov": 0.05,
                                  "use_nesterov_acc": True, "acc_rate_cov": 0.5,
                                  "delta_rel_conv": np.finfo(float).eps**0.25, "trace": False})
...
***Using  R***
```{r comparison, results='hold', message=F, cache=save_cache, eval=T}
...
gp_model <- fitGPModel(gp_coords = coords, cov_function = "exponential",
                        ...
                        params = list(optimizer_cov = "gradient_descent",
                                      ...
                                      acc_rate_cov = 0.5, delta_rel_conv = .Machine$double.eps^0.25))
...
*Note that estimating such a random effects structure is computationally rather intensive, also in the linear case, as the two grouped random effects are crossed and the corresponding covariance matrix is rather dense. Consequently, training is relatively slow for the GPBoost algorithm. If you use only one grouped random effects, training is much faster as the covariance matrix is then much sparser, and the GPBoost library relies on sparse matrix calculations.*

We first simulate data and then show how to train the GPBoost model and make predictions.
<div class="fold s">
```{r simulate_group_data, results='hide', message=F, cache=save_cache}
# Simulate data
set.seed(1)
...
***Using  R***
```{r train_group_model, results='hold', message=F, cache=save_cache, eval=run_group_re}
...
re_params <- list(optimizer_cov = "fisher_scoring", trace = FALSE,
                  delta_rel_conv = .Machine$double.eps^0.25)
