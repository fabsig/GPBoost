..  List of parameters is auto generated by GPBoost\helpers\parameter_generator.py from GPBoost\include\LightGBM\config.h file.
...
This page contains descriptions of all parameters in GPBoost.
...
-  ``num_iterations`` :raw-html:`<a id="num_iterations" title="Permalink to this parameter" href="#num_iterations">&#x1F517;&#xFE0E;</a>`, default = ``100``, type = int, aliases: ``num_iteration``, ``n_iter``, ``num_tree``, ``num_trees``, ``num_round``, ``num_rounds``, ``num_boost_round``, ``n_estimators``, constraints: ``num_iterations >= 0``
   ...
   -  **Note**: internally, GPBoost constructs ``num_class * num_iterations`` trees for multi-class classification problems
...
-  ``device_type`` :raw-html:`<a id="device_type" title="Permalink to this parameter" href="#device_type">&#x1F517;&#xFE0E;</a>`, default = ``cpu``, type = enum, options: ``cpu``, ``gpu``, aliases: ``device``
   ...
   -  **Note**: refer to `Installation Guide <./Installation-Guide.rst#build-gpu-version>`__ to build GPBoost with GPU support
...
-  ``deterministic`` :raw-html:`<a id="deterministic" title="Permalink to this parameter" href="#deterministic">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  when you use the different seeds, different GPBoost versions, the binaries compiled by different compilers, or in different systems, the results are expected to be different
...
-  ``use_nesterov_acc`` :raw-html:`<a id="use_nesterov_acc" title="Permalink to this parameter" href="#use_nesterov_acc">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  set this to ``true`` to do boosting with Nesterov acceleration

   -  can currently only be used for tree_learner = "serial"

-  ``nesterov_acc_rate`` :raw-html:`<a id="nesterov_acc_rate" title="Permalink to this parameter" href="#nesterov_acc_rate">&#x1F517;&#xFE0E;</a>`, default = ``0.5``, type = double

   -  acceleration rate for momentum step in case Nesterov accelerated boosting is used (default = 0.5)

-  ``momentum_offset`` :raw-html:`<a id="momentum_offset" title="Permalink to this parameter" href="#momentum_offset">&#x1F517;&#xFE0E;</a>`, default = ``0``, type = int

   -  acceleration rate is zero before the offset number (default = 0)
...
-  ``force_col_wise`` :raw-html:`<a id="force_col_wise" title="Permalink to this parameter" href="#force_col_wise">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  **Note**: when both ``force_col_wise`` and ``force_row_wise`` are ``false``, GPBoost will firstly try them both, and then use the faster one. To remove the overhead of testing set the faster one to ``true`` manually
...
-  ``bagging_freq`` :raw-html:`<a id="bagging_freq" title="Permalink to this parameter" href="#bagging_freq">&#x1F517;&#xFE0E;</a>`, default = ``0``, type = int, aliases: ``subsample_freq``
   ...
   -  ``0`` means disable bagging; ``k`` means perform bagging at every ``k`` iteration. Every ``k``-th iteration, GPBoost will randomly select ``bagging_fraction * 100 %`` of the data to use for the next ``k`` iterations
...
-  ``feature_fraction`` :raw-html:`<a id="feature_fraction" title="Permalink to this parameter" href="#feature_fraction">&#x1F517;&#xFE0E;</a>`, default = ``1.0``, type = double, aliases: ``sub_feature``, ``colsample_bytree``, constraints: ``0.0 < feature_fraction <= 1.0``
   ...
   -  GPBoost will randomly select a subset of features on each iteration (tree) if ``feature_fraction`` is smaller than ``1.0``. For example, if you set it to ``0.8``, GPBoost will select 80% of features before training each tree
...
-  ``feature_fraction_bynode`` :raw-html:`<a id="feature_fraction_bynode" title="Permalink to this parameter" href="#feature_fraction_bynode">&#x1F517;&#xFE0E;</a>`, default = ``1.0``, type = double, aliases: ``sub_feature_bynode``, ``colsample_bynode``, constraints: ``0.0 < feature_fraction_bynode <= 1.0``
   ...
   -  GPBoost will randomly select a subset of features on each tree node if ``feature_fraction_bynode`` is smaller than ``1.0``. For example, if you set it to ``0.8``, GPBoost will select 80% of features at each tree node
...
-  ``extra_trees`` :raw-html:`<a id="extra_trees" title="Permalink to this parameter" href="#extra_trees">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  if set to ``true``, when evaluating node splits GPBoost will check only one randomly-chosen threshold for each feature
...
-  ``first_metric_only`` :raw-html:`<a id="first_metric_only" title="Permalink to this parameter" href="#first_metric_only">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  GPBoost allows you to provide multiple evaluation metrics. Set this to ``true``, if you want to use only the first metric for early stopping
...
-  ``max_cat_threshold`` :raw-html:`<a id="max_cat_threshold" title="Permalink to this parameter" href="#max_cat_threshold">&#x1F517;&#xFE0E;</a>`, default = ``32``, type = int, constraints: ``max_cat_threshold > 0``
   ...
   -  limit number of split points considered for categorical features. See `the documentation on how GPBoost finds optimal splits for categorical features <./Features.rst#optimal-split-for-categorical-features>`_ for more details
...
-  ``verbosity`` :raw-html:`<a id="verbosity" title="Permalink to this parameter" href="#verbosity">&#x1F517;&#xFE0E;</a>`, default = ``1``, type = int, aliases: ``verbose``
   ...
   -  controls the level of GPBoost's verbosity
...
-  ``output_model`` :raw-html:`<a id="output_model" title="Permalink to this parameter" href="#output_model">&#x1F517;&#xFE0E;</a>`, default = ``GPBoost_model.txt``, type = string, aliases: ``model_output``, ``model_out``
...
-  ``max_bin`` :raw-html:`<a id="max_bin" title="Permalink to this parameter" href="#max_bin">&#x1F517;&#xFE0E;</a>`, default = ``255``, type = int, constraints: ``max_bin > 1``
   ...
   -  GPBoost will auto compress memory according to ``max_bin``. For example, GPBoost will use ``uint8_t`` for feature value if ``max_bin=255``
...
-  ``feature_pre_filter`` :raw-html:`<a id="feature_pre_filter" title="Permalink to this parameter" href="#feature_pre_filter">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool

   -  set this to ``true`` to tell GPBoost to ignore the features that are unsplittable based on ``min_data_in_leaf``
...
-  ``two_round`` :raw-html:`<a id="two_round" title="Permalink to this parameter" href="#two_round">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool, aliases: ``two_round_loading``, ``use_two_round_loading``
   ...
   -  by default, GPBoost will map data file to memory and load features from memory. This will provide faster data loading speed, but may cause run out of memory error when the data file is very big
...
-  ``predict_disable_shape_check`` :raw-html:`<a id="predict_disable_shape_check" title="Permalink to this parameter" href="#predict_disable_shape_check">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  control whether or not GPBoost raises an error when you try to predict on data with a different number of features than the training data
   ...
   -  if ``true``, GPBoost will attempt to predict on whatever data you provide. This is dangerous because you might get incorrect predictions, but you could use it in situations where it is difficult or expensive to generate some features and you are very confident that they were never chosen for splits in the model
...
-  ``output_result`` :raw-html:`<a id="output_result" title="Permalink to this parameter" href="#output_result">&#x1F517;&#xFE0E;</a>`, default = ``GPBoost_predict_result.txt``, type = string, aliases: ``predict_result``, ``prediction_result``, ``predict_name``, ``prediction_name``, ``pred_name``, ``name_pred``
