...
def test_binary():
    ...
    gbm = gpb.GPBoostClassifier(n_estimators=50, silent=True)
...
def test_regression():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=42)
    gbm = gpb.GPBoostRegressor(n_estimators=50, silent=True)
    gbm.fit(X_train, y_train, eval_set=[(X_test, y_test)], early_stopping_rounds=5, verbose=False)
...
def test_lambdarank():
    ...
    gbm = gpb.GPBoostRanker(n_estimators=50)
    gbm.fit(X_train, y_train, group=q_train, eval_set=[(X_test, y_test)],
            eval_group=[q_test], eval_at=[1, 3], early_stopping_rounds=10, verbose=False,
            callbacks=[gpb.reset_parameter(learning_rate=lambda x: max(0.01, 0.1 - 0.01 * x))])
...
def test_xendcg():
    ...
    gbm = gpb.GPBoostRanker(n_estimators=50, objective='rank_xendcg', random_state=5, n_jobs=1)
    gbm.fit(X_train, y_train, group=q_train, eval_set=[(X_test, y_test)],
            eval_group=[q_test], eval_at=[1, 3], early_stopping_rounds=10, verbose=False,
            eval_metric='ndcg',
            callbacks=[gpb.reset_parameter(learning_rate=lambda x: max(0.01, 0.1 - 0.01 * x))])
...
def test_stacking_classifier():
    ...
    classifiers = [('gbm1', gpb.GPBoostClassifier(n_estimators=3)),
                   ('gbm2', gpb.GPBoostClassifier(n_estimators=3))]
    clf = StackingClassifier(estimators=classifiers,
                             final_estimator=gpb.GPBoostClassifier(n_estimators=3),
                             passthrough=True)
...
def test_stacking_regressor():
    ...
    X, y = load_boston(return_X_y=True)
    X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=42)
    regressors = [('gbm1', gpb.GPBoostRegressor(n_estimators=3)),
                  ('gbm2', gpb.GPBoostRegressor(n_estimators=3))]
    reg = StackingRegressor(estimators=regressors,
                            final_estimator=gpb.GPBoostRegressor(n_estimators=3),
                            passthrough=True)
...
def test_grid_search():
    ...
    grid = GridSearchCV(estimator=gpb.GPBoostClassifier(**params), param_grid=grid_params,
                        cv=2)
...
def test_random_search():
    ...
    fit_params = dict(verbose=False,
                      ...
                      eval_metric=constant_metric,
                      early_stopping_rounds=2)
    rand = RandomizedSearchCV(estimator=gpb.GPBoostClassifier(**params),
...
def test_multioutput_classifier():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1,
                                                        random_state=42)
    clf = MultiOutputClassifier(estimator=gpb.GPBoostClassifier(n_estimators=10))
    clf.fit(X_train, y_train)
    score = clf.score(X_test, y_test)
    assert score >= 0.2
    assert score <= 1.
    ...
    for classifier in clf.estimators_:
        assert isinstance(classifier, gpb.GPBoostClassifier)
        assert isinstance(classifier.booster_, gpb.Booster)
...
def test_multioutput_regressor():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1,
                                                        random_state=42)
    reg = MultiOutputRegressor(estimator=gpb.GPBoostRegressor(n_estimators=10))
    reg.fit(X_train, y_train)
    ...
    for regressor in reg.estimators_:
        assert isinstance(regressor, gpb.GPBoostRegressor)
        assert isinstance(regressor.booster_, gpb.Booster)
...
def test_clone_and_property():
    X, y = load_boston(return_X_y=True)
    gbm = gpb.GPBoostRegressor(n_estimators=10, silent=True)
    gbm.fit(X, y, verbose=False)

    gbm_clone = clone(gbm)
    assert isinstance(gbm.booster_, gpb.Booster)
    assert isinstance(gbm.feature_importances_, np.ndarray)

    X, y = load_digits(n_class=2, return_X_y=True)
    clf = gpb.GPBoostClassifier(n_estimators=10, silent=True)
    clf.fit(X, y, verbose=False)
    assert sorted(clf.classes_) == [0, 1]
    assert clf.n_classes_ == 2
    assert isinstance(clf.booster_, gpb.Booster)
...
def test_random_state_object():
    ...
    clf1 = gpb.GPBoostClassifier(n_estimators=10, subsample=0.5, subsample_freq=1, random_state=state1)
    clf2 = gpb.GPBoostClassifier(n_estimators=10, subsample=0.5, subsample_freq=1, random_state=state2)
...
def test_feature_importances_single_leaf():
    ...
    clf = gpb.GPBoostClassifier(n_estimators=10)
...
def test_pandas_categorical():
    ...
    gbm0 = gpb.sklearn.GPBoostClassifier(n_estimators=10).fit(X, y)
    pred0 = gbm0.predict(X_test, raw_score=True)
    pred_prob = gbm0.predict_proba(X_test)[:, 1]
    gbm1 = gpb.sklearn.GPBoostClassifier(n_estimators=10).fit(X, pd.Series(y), categorical_feature=[0])
    pred1 = gbm1.predict(X_test, raw_score=True)
    gbm2 = gpb.sklearn.GPBoostClassifier(n_estimators=10).fit(X, y, categorical_feature=['A'])
    pred2 = gbm2.predict(X_test, raw_score=True)
    gbm3 = gpb.sklearn.GPBoostClassifier(n_estimators=10).fit(X, y, categorical_feature=['A', 'B', 'C', 'D'])
    pred3 = gbm3.predict(X_test, raw_score=True)
    gbm3.booster_.save_model('categorical.model')
    gbm4 = gpb.Booster(model_file='categorical.model')
    pred4 = gbm4.predict(X_test)
    gbm5 = gpb.sklearn.GPBoostClassifier(n_estimators=10).fit(X, y, categorical_feature=['A', 'B', 'C', 'D', 'E'])
    pred5 = gbm5.predict(X_test, raw_score=True)
    gbm6 = gpb.sklearn.GPBoostClassifier(n_estimators=10).fit(X, y, categorical_feature=[])
...
def test_predict():
    ...
    gbm = gpb.train({'objective': 'multiclass',
                     'num_class': 3,
                     'verbose': -1},
                    gpb.Dataset(X_train, y_train))
    clf = gpb.GPBoostClassifier(verbose=-1).fit(X_train, y_train)
...
def test_metrics():
    ...
    # default metric
    gbm = gpb.GPBoostRegressor(**params).fit(**params_fit)
    assert len(gbm.evals_result_['training']) == 1
    assert 'l2' in gbm.evals_result_['training']

    # non-default metric
    gbm = gpb.GPBoostRegressor(metric='mape', **params).fit(**params_fit)
    assert len(gbm.evals_result_['training']) == 1
    assert 'mape' in gbm.evals_result_['training']

    # no metric
    gbm = gpb.GPBoostRegressor(metric='None', **params).fit(**params_fit)
    assert gbm.evals_result_ is None

    # non-default metric in eval_metric
    gbm = gpb.GPBoostRegressor(**params).fit(eval_metric='mape', **params_fit)
    assert len(gbm.evals_result_['training']) == 2
    assert 'l2' in gbm.evals_result_['training']
    assert 'mape' in gbm.evals_result_['training']

    # non-default metric with non-default metric in eval_metric
    gbm = gpb.GPBoostRegressor(metric='gamma', **params).fit(eval_metric='mape', **params_fit)
    assert len(gbm.evals_result_['training']) == 2
    assert 'gamma' in gbm.evals_result_['training']
    assert 'mape' in gbm.evals_result_['training']

    # non-default metric with multiple metrics in eval_metric
    gbm = gpb.GPBoostRegressor(metric='gamma',
    ...
    # non-default metric with multiple metrics in eval_metric for GPBoostClassifier
    ...
    params_fit_classification = {'X': X_classification, 'y': y_classification,
                                 ...
                                 'verbose': False}
    gbm = gpb.GPBoostClassifier(**params_classification).fit(eval_metric=['fair', 'error'],
    ...
    gbm = gpb.GPBoostRegressor(objective='regression_l1', **params).fit(**params_fit)
    assert len(gbm.evals_result_['training']) == 1
    assert 'l1' in gbm.evals_result_['training']

    # non-default metric for non-default objective
    gbm = gpb.GPBoostRegressor(objective='regression_l1', metric='mape',
                            **params).fit(**params_fit)
    assert len(gbm.evals_result_['training']) == 1
    assert 'mape' in gbm.evals_result_['training']

    # no metric
    gbm = gpb.GPBoostRegressor(objective='regression_l1', metric='None',
                            **params).fit(**params_fit)
    assert gbm.evals_result_ is None

    # non-default metric in eval_metric for non-default objective
    gbm = gpb.GPBoostRegressor(objective='regression_l1',
    ...
    gbm = gpb.GPBoostRegressor(objective='regression_l1', metric='gamma',
                            **params).fit(eval_metric='mape', **params_fit)
    assert len(gbm.evals_result_['training']) == 2
    assert 'gamma' in gbm.evals_result_['training']
    assert 'mape' in gbm.evals_result_['training']
    ...
    gbm = gpb.GPBoostRegressor(**params).fit(eval_metric=constant_metric, **params_fit)
    assert len(gbm.evals_result_['training']) == 2
    assert 'l2' in gbm.evals_result_['training']
    assert 'error' in gbm.evals_result_['training']

    # non-default metric with custom metric
    gbm = gpb.GPBoostRegressor(metric='mape',
    ...
    gbm = gpb.GPBoostRegressor(metric=['l1', 'gamma'],
                            **params).fit(eval_metric=constant_metric, **params_fit)
    assert len(gbm.evals_result_['training']) == 3
    assert 'l1' in gbm.evals_result_['training']
    assert 'gamma' in gbm.evals_result_['training']
    ...
    gbm = gpb.GPBoostRegressor(metric='None',
                            **params).fit(eval_metric=constant_metric, **params_fit)
    assert len(gbm.evals_result_['training']) == 1
    assert 'error' in gbm.evals_result_['training']
    ...
    gbm = gpb.GPBoostRegressor(objective='regression_l1', metric=['l1', 'gamma'],
                            **params).fit(eval_metric=constant_metric, **params_fit)
    assert len(gbm.evals_result_['training']) == 3
    assert 'l1' in gbm.evals_result_['training']
    assert 'gamma' in gbm.evals_result_['training']
    ...
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric='binary_error', **params_fit)
    assert len(gbm.evals_result_['training']) == 2
    ...
    gbm = gpb.GPBoostClassifier(objective='invalid_obj',
    ...
    # and invalid binary metric is replaced with multiclass alternative
    gbm = gpb.GPBoostClassifier(objective='ovr',
                             **params).fit(eval_metric='binary_error', **params_fit)
    ...
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric='multi_error', **params_fit)
    assert len(gbm.evals_result_['training']) == 2
    assert 'binary_logloss' in gbm.evals_result_['training']
    assert 'binary_error' in gbm.evals_result_['training']
...
def test_multiple_eval_metrics():
    ...
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric=[constant_metric, decreasing_metric], **params_fit)
    assert len(gbm.evals_result_['training']) == 3
    assert 'error' in gbm.evals_result_['training']
    ...
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric=[constant_metric, decreasing_metric, 'fair'], **params_fit)
    assert len(gbm.evals_result_['training']) == 4
    assert 'error' in gbm.evals_result_['training']
    assert 'decreasing_metric' in gbm.evals_result_['training']
    assert 'binary_logloss' in gbm.evals_result_['training']
    ...
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric=[], **params_fit)
    assert len(gbm.evals_result_['training']) == 1
    assert 'binary_logloss' in gbm.evals_result_['training']

    # Verify that can receive a list of metrics, only built-in
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric=['fair', 'error'], **params_fit)
    assert len(gbm.evals_result_['training']) == 3
    assert 'binary_logloss' in gbm.evals_result_['training']

    # Verify that eval_metric is robust to receiving a list with None
    gbm = gpb.GPBoostClassifier(**params).fit(eval_metric=['fair', 'error', None], **params_fit)
    assert len(gbm.evals_result_['training']) == 3
    assert 'binary_logloss' in gbm.evals_result_['training']
...
def test_class_weight():
    ...
    gbm = gpb.GPBoostClassifier(n_estimators=10, class_weight='balanced', silent=True)
    gbm.fit(X_train, y_train,
    ...
    gbm_str = gpb.GPBoostClassifier(n_estimators=10, class_weight='balanced', silent=True)
...
def test_continue_training_with_model():
    X, y = load_digits(n_class=3, return_X_y=True)
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=42)
    init_gbm = gpb.GPBoostClassifier(n_estimators=5).fit(X_train, y_train, eval_set=(X_test, y_test),
                                                      verbose=False)
    gbm = gpb.GPBoostClassifier(n_estimators=5).fit(X_train, y_train, eval_set=(X_test, y_test),
...
def test_check_is_fitted():
    X, y = load_digits(n_class=2, return_X_y=True)
    est = gpb.GPBoostModel(n_estimators=5, objective="binary")
    clf = gpb.GPBoostClassifier(n_estimators=5)
    reg = gpb.GPBoostRegressor(n_estimators=5)
    rnk = gpb.GPBoostRanker(n_estimators=5)
    models = (est, clf, reg, rnk)
    for model in models:
        with pytest.raises(gpb.compat.GPBoostNotFittedError):
...
def _tested_estimators():
    for Estimator in [gpb.sklearn.GPBoostClassifier, gpb.sklearn.GPBoostRegressor]:
        yield Estimator()
