...
class TestBasic(unittest.TestCase):
    ...
    def test(self):
        ...
        bst = gpb.Booster(params, model_file="model.txt")
        os.remove("model.txt")
        ...
        np.testing.assert_raises_regex(gpb.basic.LightGBMError, bad_shape_error_msg,
    ...
    def test_subset_group(self):
        ...
        lgb_train = gpb.Dataset(X_train, y_train, group=q_train)
        self.assertEqual(len(lgb_train.get_group()), 201)
        subset = lgb_train.subset(list(gpb.compat.range_(10))).construct()
    ...
    def test_add_features_throws_if_num_data_unequal(self):
        ...
        with self.assertRaises(gpb.basic.LightGBMError):
    ...
    def test_get_feature_penalty_and_monotone_constraints(self):
        ...
        d = gpb.Dataset(X, params={'feature_penalty': [0.5],
    ...
    def test_add_features_feature_penalty(self):
        ...
        for (p1, p2, expected) in test_cases:
            ...
            d1 = gpb.Dataset(X[:, 0].reshape((-1, 1)), params=params1).construct()
            params2 = {'feature_penalty': p2} if p2 is not None else {}
            d2 = gpb.Dataset(X[:, 1].reshape((-1, 1)), params=params2).construct()
            d1.add_features_from(d2)
    ...
    def test_consistent_state_for_dataset_fields(self):
        ...
        sequence[0] = np.nan
        sequence[1] = np.inf
        lgb_data = gpb.Dataset(X, sequence, weight=sequence, init_score=sequence).construct()
        check_asserts(lgb_data)
        lgb_data = gpb.Dataset(X, y).construct()
