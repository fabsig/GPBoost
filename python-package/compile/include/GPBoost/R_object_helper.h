...
inline char* R_CHAR_PTR(LGBM_SE x) {
	if (R_IS_NULL(x)) {
		return nullptr;
	}
	else {
		return ((char *) DATAPTR(x));
	}
}

//#define R_INT_PTR(x)  ((int *) DATAPTR(x))
inline int* R_INT_PTR(LGBM_SE x) {
	if (R_IS_NULL(x)) {
		return nullptr;
	}
	else {
...
inline int64_t* R_INT64_PTR(LGBM_SE x) {
	if (R_IS_NULL(x)) {
		return nullptr;
	}
	else {
...
inline double* R_REAL_PTR(LGBM_SE x) {
	if (R_IS_NULL(x)) {
		return nullptr;
	}
	else {
...
#define R_AS_DOUBLE(x) (*((double *) DATAPTR(x)))

#define R_AS_BOOL(x) (*((bool *) DATAPTR(x)))
