    ...
    if (objective == nullptr) {
      ...
      if (weights_ == nullptr) {
        ...
        for (data_size_t i = 0; i < num_data_; ++i) {
          ...
          if (objective->HasGPModel() && objective->UseGPModelForValidation()) {
            t -= minus_gp_pred[i];//minus since the GP model uses score - label = -(label-score) from the last iteration to make predictions
          }
