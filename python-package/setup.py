...
GPBOOST_OPTIONS = [
    ...
    ('cudagp', None, 'Compile CUDA version for GP'),
...
def compile_cpp(
    ...
    if use_gpu:
        ...
        cmake_cmd.append("-DUSE_CUDA_GP=ON")
    if use_mpi:
    ...
    if system() in {'Windows', 'Microsoft'}:
        if use_mingw:
            ...
            ## Installion using VS solution currently not supported
            # if not any((use_gpu, use_cuda, use_cudagp, use_mpi, use_hdfs, nomp, bit32, integrated_opencl)):
...
class CustomInstall(install):
    ...
    def run(self) -> None:
        ...
        if not self.precompile:
            ...
            compile_cpp(use_mingw=self.mingw, use_gpu=self.gpu, use_cuda=self.cuda, use_cudagp=self.cudagp, use_mpi=self.mpi,
