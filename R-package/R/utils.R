gpb.is.Booster <- function(x) {
  gpb.check.r6.class(x, "gpb.Booster") # Checking if it is of class gpb.Booster or not
}

gpb.is.Dataset <- function(x) {
  gpb.check.r6.class(x, "gpb.Dataset") # Checking if it is of class gpb.Dataset or not
}
...
gpb.encode.char <- function(arr, len) {
  ...
  if (!is.raw(arr)) {
    stop("gpb.encode.char: Can only encode from raw type") # Not an object of type raw
  }
...
gpb.call <- function(fun_name, ret, ...) {
  ...
  if (!is.null(ret)) {
    if (is.character(fun_name)) {
      call_state <- .Call(fun_name, ..., ret, call_state, PACKAGE = "lib_gpboost") # Call with ret
    } else {
      call_state <- .Call(fun_name, ..., ret, call_state) # Call with ret
    }
  } else {
    if (is.character(fun_name)) {
      call_state <- .Call(fun_name, ..., call_state, PACKAGE = "lib_gpboost") # Call without ret
    } else {
      call_state <- .Call(fun_name, ..., call_state) # Call without ret
    }
  }
  ...
  if (call_state != 0L) {
    ...
    act_len <- 0L
    err_msg <- raw(buf_len)
    err_msg <- .Call("LGBM_GetLastError_R", buf_len, act_len, err_msg, PACKAGE = "lib_gpboost")
    ...
    # Return error
    stop("api error: ", gpb.encode.char(err_msg, act_len))
...
gpb.call.return.str <- function(fun_name, ...) {
  ...
  # Call buffer
  buf <- gpb.call(fun_name, ret = buf, ..., buf_len, act_len)
  ...
  return(gpb.encode.char(buf, act_len))
...
gpb.check.params <- function(params) {
...
gpb.check.obj <- function(params, obj) {
  ...
  if (is.character(params$objective)) {
    ...
    if (!(params$objective %in% OBJECTIVES)) {
      ...
      stop("gpb.check.obj: objective name error should be one of (", paste0(OBJECTIVES, collapse = ", "), ")")
