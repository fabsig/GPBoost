#' Parse a GPBoost tree model json dump
#'
#' Parse a GPBoost tree model json dump into a \code{data.table} structure.
#'
...
#' @export
boost.model.dt.tree <- function(model, num_iteration = NULL) {

  # Dump json model first
  json_model <- gpb.dump(model, num_iteration = num_iteration)
