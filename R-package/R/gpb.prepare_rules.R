...
#'
#' Attempts to prepare a clean dataset to prepare to put in a \code{gpb.Dataset}. Factors and characters are converted to numeric. In addition, keeps rules created so you can convert other datasets using this converter.
#'
...
#' str(iris)
#'
#' new_iris <- gpb.prepare_rules(data = iris) # Autoconverter
#' str(new_iris$data)
#'
...
#' newer_iris <- gpb.prepare_rules(data = iris, rules = new_iris$rules)
#'
...
#'                                    "versicolor" = 2,
#'                                    "virginica" = 1))
#' newest_iris <- gpb.prepare_rules(data = iris, rules = personal_rules)
#' str(newest_iris$data) # SUCCESS!
#'
#' @importFrom data.table set
#' @export
gpb.prepare_rules <- function(data, rules = NULL) {
  ...
  if (inherits(data, "data.table")) {
    ...
    if (!is.null(rules)) {
      ...
      if (inherits(data, "data.frame")) {
        ...
        stop("gpb.prepare: you provided ", paste(class(data), collapse = " & "), " but data should have class data.frame")
