if(R.Version()$arch != "i386"){##32-bit version is not supported
  ...
  test_that("Feature penalties work properly", {
    ...
    var_gain <- lapply(bst, function(x) feature.importance(x)[Feature == var_name, Gain])
    var_cover <- lapply(bst, function(x) feature.importance(x)[Feature == var_name, Cover])
    var_freq <- lapply(bst, function(x) feature.importance(x)[Feature == var_name, Frequency])
