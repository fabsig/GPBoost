...
if(R.Version()$arch != "i386"){##32-bit version is not supported
    ...
    test_that("plotInterpretation works as expected for binary classification", {
        ...
        num_trees <- 5
        tree_interpretation <- interprete(
            model = model
            , data = test$data
            , idxset = 1:num_trees
        )
    ...
    test_that("plotInterpretation works as expected for multiclass classification", {
        data(iris)
        ...
        dtrain <- gpb.Dataset(data = train[, 1:4], label = train[, 5])
        dtest <- gpb.Dataset.create.valid(dtrain, data = test[, 1:4], label = test[, 5])
        params <- list(
