...
SEXP GPB_GetNumCGSteps_R(SEXP handle,
	SEXP num_cg_steps) {
	R_API_BEGIN();
	CHECK_CALL(GPB_GetNumCGSteps(R_ExternalPtrAddr(handle),
		R_INT_PTR(num_cg_steps)));
	R_API_END();
	return R_NilValue;
}

SEXP GPB_GetNumCGStepsTridiag_R(SEXP handle,
	SEXP num_cg_steps) {
	R_API_BEGIN();
