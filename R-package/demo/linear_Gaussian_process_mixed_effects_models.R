...
gp_model <- fitGPModel(group_data = group, y = y,
                       params = list(use_nesterov_acc = FALSE, trace=TRUE, std_dev=TRUE))
summary(gp_model)
...
set.seed(1)
X <- cbind(rep(1,n),runif(n)) # desing matrix / covariate data for fixed effects
beta <- c(3,3) # regression coefficents
y <- eps + xi + X%*%beta # add fixed effects to observed data
# Create random effects model
gp_model <- GPModel(group_data = group)
# Fit model
...
gp_model <- fitGPModel(group_data=group_data, y=y, params = list(std_dev = TRUE))
summary(gp_model)
