...
TEMP_R_DIR <- file.path(getwd(), "gpboost_r")
...
for (eigen_module in c(eigen_modules, "misc", "plugins")) {
  if (eigen_module %in% c("Dense","Sparse")) {
    next
  }
...
#
# useDynLib(lib_gpboost, LGBM_GetLastError_R, LGBM_DatasetCreateFromFile_R, ...)
#
